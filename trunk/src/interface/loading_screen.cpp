  ...
  icon_count(icon_count)
{
  ...
  loading_bg->ScaleSize(GetRenderer().GetSize());
...
void LoadingScreen::DrawBackground()
{
  ...
  loading_bg->ScaleSize(window.GetSize());
  loading_bg->Blit(window, 0, 0);

  Point2i loading_text_pos(window.GetWidth()/2,
                                 ...
                                 const std::string& label) const
{
  ...
  int x = ((window.GetWidth() - icon_count*120)/2)+ index*120;
  int y = (window.GetHeight()/2)+40;

  Rectanglei dest (x+slot_margin_x, y, image.GetWidth(), image.GetHeight());
  window.Blit(image, dest.GetPosition());
